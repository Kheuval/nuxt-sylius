...
import { normalizeListProducts } from "~/src/Product/server/normalizer/ListProducts";
import { normalizeSingleProduct } from "~/src/Product/server/normalizer/SingleProduct";
...
export const useProductRepository = () => {
  const findLatest = async (options?: { locale?: string; count?: number }) =>
    normalizeListProducts(await findLatestQuery(options));

  const findOneBySlug = async (options: { slug: string; locale?: string }) =>
    normalizeSingleProduct(await findOneBySlugQuery(options));
